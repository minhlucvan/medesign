...
function screenshotUrl(p: string): string {
  return `${process.env.MEDESIGN_BACKEND_URL ?? 'http://localhost:4321'}/screenshots/${path.basename(p)}`;
}

/**
 * Screenshots a component's Storybook story and diffs it against the stored baseline.
 *
 * This is medesign's "visual testing in the loop": after each change request, the agent
