/**
 * File watcher for Claude Code session files — adapted from claude-run (MIT).
 * Uses chokidar to watch ~/.claude/history.jsonl and ~/.claude/projects/ for changes.
 */
...
export function startWatcher(): void {
  ...
  const usePolling = process.env.EMDESIGN_USE_POLLING === "1";
  ...
  watcher.on("error", (error) => {
    console.error("[emdesign/session] Watcher error:", error);
  });
}
