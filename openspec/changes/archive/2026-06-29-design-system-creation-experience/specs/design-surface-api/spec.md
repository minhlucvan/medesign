...
- `dsWorkflowStatus`: Object describing any ongoing design system creation workflow, or null if no workflow is active. Format: `{ active: boolean, stage: string | null, status: 'running' | 'done' | 'error', progress: number }`
...
#### Scenario: DS workflow in progress
- **WHEN** a design system creation workflow is running
- **THEN** the surface response includes `dsWorkflowStatus: { active: true, stage: "Generating Tokens", status: "running", progress: 0.6 }`
- **WHEN** the workflow completes
- **THEN** `dsWorkflowStatus` returns `{ active: false, stage: "Complete", status: "done", progress: 1.0 }` for the next 5 seconds, then null
